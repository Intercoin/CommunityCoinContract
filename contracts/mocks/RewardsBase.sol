...
abstract contract RewardsBase is TrustedForwarder, OwnableUpgradeable {
    ...
    function _exchange(uint256 inputAmount) internal {
        ...
        require(
            amount2send > 0,
            "FundContract: Can not calculate amount of tokens"
        );

        uint256 tokenBalance = IERC20(sellingToken).balanceOf(address(this));
        require(
            tokenBalance >= amount2send,
            "FundContract: Amount exceeds allowed balance"
        );
     ...
     */
    function setGroup(address[] memory addresses, string memory groupName)
        public
        onlyOwner
    {
     ...
     */
    function getGroupBonus(string memory groupName)
        public
        view
    ...
    function _claim(address addr, string memory groupName) internal {
        ...
        uint256 participantTotalBonusTokens = (_getTokenAmount(
            participants[addr].totalAmount,
            tokenPrice
        ) * groupBonus) / 1e2;

        if (participantTotalBonusTokens > participants[addr].contributed) {
            uint256 amount2Send = participantTotalBonusTokens -
                participants[addr].contributed;
    ...
    function _getGroupBonus(string memory groupName)
        internal
        view
        returns (uint256 bonus)
    {
     ...
     */
    function _getTokenAmount(uint256 amount, uint256 price)
        internal
        pure
        returns (uint256)
    {
